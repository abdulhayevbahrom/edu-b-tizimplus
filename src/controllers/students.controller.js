...
async function buildStudentFilter(query, user) {
  ...
  if (user?.role === "teacher" && user.teacherId) {
    const teacherGroupIds = await Group.find({
      teacherId: user.teacherId,
    }).distinct("_id");
    conditions.push({
      $or: [
        { groupId: { $in: teacherGroupIds } },
        { "enrollments.groupId": { $in: teacherGroupIds } },
      ],
    });
  }
  ...
  if (query.groupId) {
    conditions.push({
      $or: [
        ...
        { "enrollments.groupId": query.groupId },
      ],
    });
  }
  ...
  if (query.view === "current" || query.view === "history") {
    const archivedGroupIds = await Group.find({ status: "archived" }).distinct(
      "_id",
    );
...
function toStudentResponse(student) {
  ...
  const enrollmentHistory = data.enrollmentHistory?.length
    ? data.enrollmentHistory.map((item) => ({
        ...
        endReason:
          item.endReason || (isArchivedGroup ? "Guruh arxivlandi" : ""),
      }))
    : group
      ? [
          {
  ...
  const enrollments = (data.enrollments || []).map((item) => {
    const enrollmentGroup =
      item.groupId && typeof item.groupId === "object"
        ? toGroupResponse(item.groupId)
        : null;
    return {
...
function getDuplicateMessage(duplicate, payload) {
  ...
  if (
    ...
    (duplicate.phone === payload.secondaryPhone ||
      duplicate.secondaryPhone === payload.secondaryPhone)
  ) {
...
export async function getStudents(req, res) {
  try {
    ...
    res
      .status(500)
      .json({
        message: "O'quvchilar ro'yxatini olishda xatolik",
        error: error.message,
      });
  }
}
  ...
  try {
    ...
    if (
      ...
      (req.user?.role === "teacher" &&
        student.groupId?.teacherId?._id?.toString() !==
          req.user.teacherId?.toString())
    ) {
    ...
    return res
      .status(500)
      .json({
        message: "O'quvchi ma'lumotini olishda xatolik",
        error: error.message,
      });
  }
}
...
export async function createStudent(req, res) {
  try {
    ...
    if (
      group.status !== "active" ||
      (group.isEnrollmentOpen === false && !payload.allowClosedGroup)
    ) {
    ...
    if (duplicate) {
      return res
        .status(409)
        .json({ message: getDuplicateMessage(duplicate, payload) });
    }
      ...
      enrollments: [
        {
          ...
          discountHistory:
            payload.discountType && payload.discountType !== "none"
              ? [
                  {
                    type: payload.discountType,
                    value: Number(payload.discountValue) || 0,
    ...
    if (error.code === 11000) {
      return res
        .status(409)
        .json({ message: "Telefon ma'lumoti takrorlangan" });
    }

    return res
      .status(400)
      .json({ message: "O'quvchi yaratishda xatolik", error: error.message });
  }
}
...
export async function updateStudent(req, res) {
  try {
    ...
    if (groupChanged) {
      const now = new Date();
      const activeEnrollment = student.enrollmentHistory.find(
        (item) => !item.endedAt,
      );
      ...
      const previousEnrollment = student.enrollments.find(
        (item) =>
          item.status === "active" &&
          item.groupId.toString() === student.groupId.toString(),
      );
    ...
    return res
      .status(400)
      .json({
        message: "O'quvchi ma'lumotini yangilashda xatolik",
        error: error.message,
      });
  }
}
...
export async function addStudentEnrollment(req, res) {
  try {
    ...
    const duplicate = student.enrollments.some(
      (item) =>
        item.status === "active" && item.groupId.toString() === group.id,
    );
    if (
      duplicate ||
      (student.groupId?.toString() === group.id && !student.enrollments.length)
    ) {
      return res
        .status(409)
        .json({ message: "O'quvchi bu guruhga allaqachon yozilgan" });
    }
    student.enrollments.push({
      groupId: group._id,
      ...
      discountHistory:
        req.body.discountType && req.body.discountType !== "none"
          ? [
              {
                type: req.body.discountType,
                value: Number(req.body.discountValue) || 0,
                reason: req.body.discountReason?.trim() || "",
                startedAt: new Date(),
              },
            ]
          : [],
    ...
    return res
      .status(400)
      .json({ message: "Kursga yozib bo‘lmadi", error: error.message });
  }
}
...
export async function updateStudentEnrollment(req, res) {
  try {
    ...
    if (!enrollment)
      return res.status(404).json({ message: "Kurs yozuvi topilmadi" });
    ...
    return res
      .status(400)
      .json({
        message: "Kurs ma’lumotini yangilab bo‘lmadi",
        error: error.message,
      });
  }
}

export async function leaveStudent(req, res) {
  try {
    const student = await Student.findById(req.params.id);
    ...
    if (student.status === "left") {
      return res.status(409).json({ message: "O'quvchi allaqachon ketgan" });
    }

    const now = new Date();
    const reason = req.body.reason?.trim() || "O'qishni to'xtatdi";
    const activeHistory = student.enrollmentHistory.find((item) => !item.endedAt);

    if (activeHistory) {
      activeHistory.endedAt = now;
      activeHistory.endReason = reason;
    } else if (!student.enrollmentHistory.length && student.groupId) {

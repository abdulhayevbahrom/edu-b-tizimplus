  ...
  try {
    ...
    res.json({
      data: teachers.map((teacher) =>
        toTeacherResponse(
          teacher,
          groupCountMap.get(teacher._id.toString()) || 0,
        ),
      ),
      pagination: {
        page,
    ...
    res
      .status(500)
      .json({
        message: "O'qituvchilar ro'yxatini olishda xatolik",
        error: error.message,
      });
  }
}
...
export async function getTeacherById(req, res) {
  try {
    ...
    return res
      .status(500)
      .json({
        message: "O'qituvchi ma'lumotini olishda xatolik",
        error: error.message,
      });
  }
}
...
export async function createTeacher(req, res) {
  try {
    ...
    if (error.code === 11000) {
      return res
        .status(409)
        .json({ message: "Telefon yoki Telegram ma'lumoti takrorlangan" });
    }

    res
      .status(400)
      .json({ message: "O'qituvchi yaratishda xatolik", error: error.message });
  }
}
...
export async function updateTeacher(req, res) {
  try {
    ...
    return res
      .status(400)
      .json({
        message: "O'qituvchi ma'lumotini yangilashda xatolik",
        error: error.message,
      });
  }
}
...
export async function deleteTeacher(req, res) {
  try {
    ...
    return res
      .status(500)
      .json({
        message: "O'qituvchini o'chirishda xatolik",
        error: error.message,
      });
  }
}
